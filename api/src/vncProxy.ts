...
async function resolveWorker(
    redis: Redis,
    jobId: string,
): Promise<{ ip: string; port: string } | null> {
...
export async function handleVncHttp(
    ...
    if (!worker) {
        return new Response("live view unavailable (job not running)", {
            status: 404,
        });
    }
...
export async function tryUpgradeVnc(
    req: Request,
    redis: Redis,
): Promise<{ jobId: string; upstreamUrl: string; workerId: string } | null> {
    ...
    return {
        jobId,
        upstreamUrl,
        workerId: `${worker.ip}:${worker.port}`,
    };
}
...
export const vncWebSocketHandlers = {
    open(ws: ServerWebSocket<WsCtx>) {
        ...
        upstream.addEventListener("open", () => {
            ...
            for (const m of ws.data.queued) {
                try {
                    ...
                    console.error("[vncProxy] queued send error:", e);
                }
            }
            ws.data.queued = [];
        });
        ...
        upstream.addEventListener("close", (ev: CloseEvent) => {
            try {
                ws.close(ev.code || 1000, ev.reason || "upstream closed");
            } catch { }
        });
    ...
    close(ws: ServerWebSocket<WsCtx>) {
        try {
            ws.data.upstream.close(1000, "client closed");
        } catch { }
    },
};
