...
//      (Using the raw HTTP body directly was the original bug: it lacks
//      fields the persisted doc carries.)
//   2. If the RC lookup FAILS (HTTP error / timeout / unusable body), fall
//      back to whatever is already cached in vehicleDetails/{REGNO}, even if
//      it's old/stale — better a slightly stale record than nothing. If the
//      DB is also empty, return null.
//
// Why API-first instead of cache-first: the live lookup is the source of
// truth and the only way to pick up changes (new owner, renewed fitness,
// etc.). The DB is now purely a resilience layer for when the RC service is
// down. This trades a little extra latency/cost on every run for always-fresh
// data; the cache is no longer served when the API is reachable.
//
...
export async function fetchVehicleDetails(
    ...
    if (apiOk) {
        ...
        // Lookup reported success but the doc never showed up. Fall through to
        // the stale-read fallback below — it'll almost certainly be null too,
        // but if an older record exists we'd rather serve it than nothing.
        console.error(
