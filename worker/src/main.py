...
EDGE_DOMAIN = os.environ.get("EDGE_DOMAIN", "automation-agent.cabswale.in")
...
async def heartbeat_loop(r: redis.Redis):
    """Refresh this worker's presence every HEARTBEAT_INTERVAL seconds."""
    meta = {
        "ip": worker_ip,
...
async def main():
    ...
    print(
        f"Orchestrator started: worker_id={worker_id} ip={worker_ip} "
        f"max_slots={MAX_SLOTS} queued={queued}"
    )
    print(
