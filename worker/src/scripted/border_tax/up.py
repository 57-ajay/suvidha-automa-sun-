...
async def run(
    ...
    if outcome == "pending_popup":
        ...
        print(f"[up.phase3] pending-tx popup detected; attempting auto-clear")
        ...
        # clear_status == "cleared": re-run phases 1+2, re-fill vehicle,
        # re-click Get Details, re-evaluate the outcome. ONE retry only —
        # if the second pass still doesn't yield district_ready we bail.
        print(f"[up.phase3] pending tx cleared; restarting phases 1+2")
