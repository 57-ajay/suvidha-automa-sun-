...
class SlotPool:
    ...
    def try_acquire(self, job_id: str) -> Slot | None:
        ...
        # Edge looks this up to route /vnc/{jobId}/* to the right worker.
        try:
            self.r.set(
                f"worker:job:{job_id}",
            ...
            print(f"[pool] WARN: could not register routing for {job_id}: {e}")
    ...
    def release(self, slot: Slot):
        ...
        if job_id:
            try:
                ...
                print(f"[pool] WARN: could not delete routing for {job_id}: {e}")
    ...
    def _start_display(self, slot: Slot) -> bool:
        ...
        for _ in range(20):
            ...
            print(
                f"[pool] Slot {slot.index}: Xvfb socket never appeared at "
                f"{socket_path}"
            )
        ...
        for _ in range(30):
            ...
            print(
                f"[pool] Slot {slot.index}: x11vnc never bound port "
                f"{slot.vnc_port}"
            )
            self._stop_display(slot)
            return False

        print(f"[pool] Slot {slot.index}: x11vnc ready on port {slot.vnc_port}")
        return True
